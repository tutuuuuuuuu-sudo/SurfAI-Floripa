    ...
    <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0a0a0a;color:#fff;border-radius:16px;overflow:hidden">
        ...
        <div style="background:#1a1a1a;border:1px solid #333;border-radius:12px;padding:24px;margin:24px 0">
          ...
          <div style="display:flex;flex-direction:column;gap:10px">
            <div style="display:flex;align-items:center;gap:10px;color:#ccc;font-size:14px">
              <span style="color:#0ea5e9;font-size:18px">🌊</span> Score de condições para 17 praias em tempo real
            </div>
            <div style="display:flex;align-items:center;gap:10px;color:#ccc;font-size:14px">
              <span style="color:#0ea5e9;font-size:18px">🤖</span> Relatório diário gerado por IA toda manhã
            </div>
            <div style="display:flex;align-items:center;gap:10px;color:#ccc;font-size:14px">
              <span style="color:#0ea5e9;font-size:18px">🔔</span> Alertas quando o mar estiver bom
            </div>
            <div style="display:flex;align-items:center;gap:10px;color:#ccc;font-size:14px">
              <span style="color:#0ea5e9;font-size:18px">🗺️</span> Navegação direta até o pico
            </div>
          </div>
        </div>

        <a href="${process.env.APP_URL ?? 'https://surf-ai-floripa.vercel.app'}"
           style="display:block;text-align:center;background:linear-gradient(135deg,#0ea5e9,#6366f1);color:#fff;text-decoration:none;padding:16px 32px;border-radius:12px;font-weight:700;font-size:16px;margin:24px 0">
      ...
      <div style="padding:20px 32px;background:#111;text-align:center">
        <p style="margin:0;font-size:12px;color:#555">
          ...
          <a href="${process.env.APP_URL ?? 'https://surf-ai-floripa.vercel.app'}/privacy" style="color:#555">Privacidade</a>
        </p>
      </div>
    </div>
  `
...
export default async function handler(req: Request) {
  ...
  // Valida o secret do webhook do Supabase
